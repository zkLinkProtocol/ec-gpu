    ...
    G::Scalar: PrimeField,
{
    ...
    let proj_size = std::mem::size_of::<<G as CurveAffine>::Projective>();
    ...
    G: CurveAffine + GpuName,
{
    ...
    pub fn multiexp(
        ...
        let exp_bits = std::mem::size_of::<<G::Engine as ScalarEngine>::Fr>() * 8;
        let window_size = self.calc_window_size(bases.len(), exp_bits);
        // windows_size * num_windows needs to be >= 256 in order for the kernel to work correctly.
        let num_windows: usize = ((exp_bits as f64) / (window_size as f64)).ceil() as usize;
        ...
        let closures = program_closures!(|program, _arg| -> EcResult<Vec<<G as CurveAffine>::Projective>> {
            ...
            let bucket_buffer = unsafe { program.create_buffer::<<G as CurveAffine>::Projective>(self.work_units * bucket_len)? };
    ...
    fn calc_window_size(&self, n: usize, exp_bits: usize) -> usize {
        // The window size was determined by running the `gpu_multiexp_consistency` test and
        // looking at the resulting numbers.
        let lower_bound = (((exp_bits * n) as f64) / (self.work_units as f64)).ln();
    ...
    G: CurveAffine + GpuName,
{
    ...
    pub fn multiexp(
        ...
        let bases = &bases_arc[skip..(skip + n)];
        let exps = &exps[..];
        ...
        pool.scoped(|s| {
            ...
            self.parallel_multiexp(s, bases, exps, &mut results, n, error.clone());
        });
